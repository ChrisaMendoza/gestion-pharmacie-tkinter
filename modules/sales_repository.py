...
class SalesRepository:
    ...
    def search_medicaments(query: str = "") -> list[tuple]:
        """
        Retourne: (id, code, nom_commercial, dosage, prix_vente, stock_actuel, date_peremption)
        """
        q = (query or "").strip()
        conn = get_connection()
        try:
    ...
    def find_client_by_phone(phone: str) -> tuple | None:
        """
        Retourne: (id, nom, prenom, telephone) ou None
        """
        p = (phone or "").strip()
        if not p:
            return None
    ...
    def create_sale(cart_lines: list[dict], remise: float = 0.0, client_id: int | None = None, user_id: int | None = None) -> int:
        ...
        - Vérifie stock
        - Crée ventes + vente_lignes
        - Décrémente medicaments.stock_actuel
        - Ajoute sorties_stock (motif VENTE)
        """
        if not cart_lines:
        ...
        try:
            cur = conn.cursor()
            # 1) Vérification stock (transaction)
            for line in cart_lines:
    ...
    def get_sale_ticket_text(vente_id: int) -> str:
        """
        Génère un ticket texte simple à partir de la BD.
        """
        conn = get_connection()
        try:
            cur = conn.cursor()
